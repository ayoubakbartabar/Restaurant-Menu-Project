...
function removeOrderFunc(foodId) {
  // حذف غذا از آرایه سبد خرید
  addFoodsArray = addFoodsArray.filter((food) => food.id != foodId);
  ...
  // نمایش یا پنهان کردن پیام سبد خرید خالی
  emptyVersion(addFoodsArray);

  // به روز رسانی دکمه‌ها و ورودی‌ها
  updateAddToCartButton(foodId);
}

function updateAddToCartButton(foodId) {
  const food = addFoodsArray.find((food) => food.id === foodId); // پیدا کردن غذا بر اساس آیدی
  const foodBoxElem = document.querySelector(`.foods-box[data-id='${foodId}']`); // پیدا کردن المان غذا
  const buyBtn = foodBoxElem.querySelector(".buying"); // گرفتن دکمه خرید
  const foodImgElem = foodBoxElem.querySelector(".foods-img"); // گرفتن تصویر غذا

  // پنهان کردن ورودی‌ها و دکمه‌ها
  let quantityBox = foodBoxElem.querySelector(".quantity-box");
  if (quantityBox) {
    quantityBox.style.display = "none"; // پنهان کردن باکس تعداد
  }

  // ساخت دکمه "Add to Cart"
  let newAddToCartBtn = document.createElement("button");
  newAddToCartBtn.classList.add("add-to-cart");
  ...
  newAddToCartBtn.append(newBuySvgElem, newBtnText);
  buyBtn.innerHTML = ""; // حذف محتوای قبلی دکمه خرید
  buyBtn.append(newAddToCartBtn); // اضافه کردن دکمه جدید به دکمه خرید
  buyBtn.style.backgroundColor = ""; // پس زمینه را به حالت اولیه باز می‌گرداند

  // بازسازی دکمه "Add to Cart"
  createAddToCartButton(food, buyBtn, foodImgElem); // بازسازی دکمه "Add to Cart"
}
